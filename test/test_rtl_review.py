...
def test_rtl_review_uses_design_scope_for_generic_rtl(tmp_path: Path, monkeypatch):
    rtl = tmp_path / "peripheral.sv"
    rtl.write_text(
        "module peripheral(input logic clk, input logic en, output logic y);\n"
        "  logic gated_clk;\n"
        "  assign gated_clk = clk & en;\n"
        "  assign y = gated_clk;\n"
        "endmodule\n",
    ...
    create_workspace(
        CreateWorkspaceData(
            directory=str(tmp_path / "ws_generic_review"),
            frontend_design_kind=GENERIC_RTL,
            parameters={"Design": "peripheral", "Top module": "peripheral"},
            rtl_list=[str(rtl)],
        )
    )
    workspace = load_workspace(str(tmp_path / "ws_generic_review"))
    ...
    assert report["scope"] == "design"
    assert report["structural_probe"]["scope"] == "design"
    assert report["structural_probe"]["mode"] == "design_yosys_precheck"
    assert all(issue["ownership"] == "design" for issue in report["issues"])
    assert report["summary"]["actionable_issues"] == len(report["issues"])
