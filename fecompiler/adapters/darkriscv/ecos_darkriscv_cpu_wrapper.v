// ECOS DarkRISCV adapter for the ysyx AXI CPU socket.
//
// DarkRISCV exposes separate instruction and data Harvard buses.  This wrapper
// arbitrates those native ports, maps DarkRISCV's low reset address alias to
// the ECOS CPU-test memory window, and presents the same socket used by the
// frontend SoC harnesses.
...
module ecos_darkriscv_cpu_wrapper (
  `ECOS_CPU_SOCKET_PORTS
);

  localparam [31:0] LOW_ALIAS_LIMIT = 32'h1000_0000;
  localparam [31:0] LOW_ALIAS_BASE = 32'h2000_0000;
  ...
  wire [31:0] instr_bus_addr = ecos_bus_addr(instr_addr);
  ...
  function [31:0] ecos_bus_addr;
    input [31:0] addr;
    begin
      ecos_bus_addr = (addr < LOW_ALIAS_LIMIT) ? (addr + LOW_ALIAS_BASE) : addr;
    end
  endfunction

  function [2:0] dark_len_to_axi_size;
    input [2:0] len;
    begin
      if (len[0]) begin
        dark_len_to_axi_size = 3'b000;
      end else if (len[1]) begin
        dark_len_to_axi_size = 3'b001;
      end else begin
        dark_len_to_axi_size = 3'b010;
      end
    end
  endfunction
        ...
        ST_IDLE: begin
          ...
          if (ready_for_new_req && !local_write) begin
            if (data_req) begin
              ...
              axi_wstrb_q <= data_be;
              axi_size_q <= dark_len_to_axi_size(data_len);
              if (data_write_req) begin
                state_q <= ST_WRITE_ADDR_DATA;
              end else if (data_read_req) begin
              ...
              serving_data_q <= 1'b0;
              axi_addr_q <= {instr_bus_addr[31:2], 2'b00};
              axi_wdata_q <= 32'b0;
              axi_wstrb_q <= 4'b0000;
              axi_size_q <= 3'b010;
              state_q <= ST_READ_ADDR;
...
`ifndef SYNTHESIS
              $fatal(1, "DarkRISCV AXI read error: resp=%0d addr=0x%08x", io_master_rresp, axi_addr_q);
`endif
            end
...
`ifndef SYNTHESIS
              $fatal(1, "DarkRISCV AXI write error: resp=%0d addr=0x%08x", io_master_bresp, axi_addr_q);
`endif
            end
            data_ack_q <= 1'b1;
            state_q <= ST_IDLE;
...
module ysyx_00000000 (
  ...
  ecos_darkriscv_cpu_wrapper adapter (
