...
_ECOS_DIFFTEST_DEFINE = "ECOS_DIFFTEST"
_DIFFTEST_IMPORT_RE = re.compile(
    r'import\s+"DPI-C"\s+function\s+int\s+difftest_step\b'
)
_FINGERPRINT_SOURCE_PATH_FIELDS = (
    "cpu_filelist",
    "soc_filelist",
    "filelist",
    "origin_verilog",
...
def workspace_input_fingerprint(workspace: dict[str, Any]) -> dict[str, str]:
    ...
    path_fields = list(_FINGERPRINT_SOURCE_PATH_FIELDS)
    if str(workspace.get("frontend_design_kind", "")).strip() == "generic_rtl":
        path_fields.append("input_filelist")
...
def _input_content_sha256(field: str, path_text: str) -> str:
    if not path_text:
        return ""
    path = Path(path_text)
    if field in {"cpu_filelist", "soc_filelist", "input_filelist", "filelist"}:
...
def rtl_sources_provide_difftest_adapter(sources: list[str | Path]) -> bool:
    """Return whether selected RTL contains the ECC difftest DPI entrypoint."""
    for source in sources:
        ...
        text = re.sub(r"//[^\r\n]*", "", text)
        if _DIFFTEST_IMPORT_RE.search(text):
            return True
    return False


def write_generated_rtl_filelist(
    filelist_path: Path,
    sources: list[str],
    *,
    enable_difftest: bool = True,
    description: str = "selected CPU RTL files",
) -> Path:
    """Write a deterministic filelist with absolute, shell-quoted RTL paths."""
    resolved: list[Path] = []
    seen: set[Path] = set()
    for source in sources:
        path = Path(source).expanduser().resolve()
    ...
    include_dirs = list(dict.fromkeys(path.parent for path in resolved))
    lines = [f"# Generated by ecc-fe from {description}"]
    if enable_difftest and rtl_sources_provide_difftest_adapter(resolved):
        lines.append(f"+define+{_ECOS_DIFFTEST_DEFINE}")
    lines.extend(f"+incdir+{shlex.quote(str(path))}" for path in include_dirs)
    lines.extend(shlex.quote(str(path)) for path in resolved)
