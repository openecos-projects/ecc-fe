...
classify_path() {
  ...
  case "${path}" in
    ...
    .github/scripts/test-release-selection.sh)
      select_all
      ;;
    bin|bin/*|packaging|packaging/*|ecc-fe.spec|fecompiler|README.md|LICENSE|pyproject.toml|uv.lock|requirements-bazel.lock|BUILD.bazel|MODULE.bazel|MODULE.bazel.lock)
